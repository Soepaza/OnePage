...
for loja in dicionario_lojas:
    ...
    try:
        ...
        nome_Onepage = "{}_{}_{}.xlsx".format(
            dia_indicador.month, dia_indicador.day, loja_formatada)
        local_arquivo = caminho_backup / loja_formatada / \
            nome_Onepage  # "C:/Users/Home/../17_02_Loja.xlsx"
...
for loja in dicionario_lojas:
    ...
    if not emails.loc[emails['Loja'] == loja].empty:
        ...
        print(
            f'E-mail da loja {loja} está vazio. Pulando para a próxima loja.')
else:
    print(
        f'Loja {loja} não encontrada no arquivo de e-mails. Pulando para a próxima loja.')
    ...
    try:
        ...
        attachment = pathlib.Path.cwd() / caminho_backup / loja / \
            f'{dia_indicador.month}_{dia_indicador.day}_{loja_formatada}.xlsx'
...
nome_arquivo = ('{dia_indicador.month}_{dia_indicador.day}_Ranking Anual.xlsx')
faturamento_lojas_ano.to_excel(
    r'Backup Arquivos Lojas \ {}'.format(nome_arquivo))
...
faturamento_lojas_dia = faturamento_lojas_dia.sort_values(
    by='Valor Final', asceding=False)

nome_arquivo = ('{dia_indicador.month}_{dia_indicador.day}_Ranking Dia.xlsx')
faturamento_lojas_dia.to_excel(
    r'Backup Arquivos Lojas \ {}'.format(nome_arquivo))

outlook = outlook.Dispatch('outlook.application')
email = outlook.CreateItem(0)
email.to = emails.loc[emails['Gerente'] == 'Diretoria', :]
email.Subject = f'''
Prezados, bom dia.

Melhor loja do dia em Faturamento {faturamento_lojas_dia.index[0]} com o faturamento de: {faturamento_lojas_dia.iloc[0, 0]}
Pior loja do dia em Faturamento {faturamento_lojas_dia.index[-1]} com o faturamento de: {faturamento_lojas_dia.iloc[-1, -1]}

Melhor loja do ano em Faturamento {faturamento_lojas_ano.index[0]} com o faturamento de: {faturamento_lojas_ano.iloc[0, 0]}
Pior loja do ano em Faturamento {faturamento_lojas_ano.index[-1]} com o faturamento de: {faturamento_lojas_ano.iloc[-1,-1]}
...
email.Attachment.Add(str(attachment))
